    ...
    IRepositorioResolucion resoluciones) : IServicioEstadisticas
{
        ...
        CancellationToken ct)
    {
        ...
        var activos = datos.Count(
            reporte => reporte.Estado != EstadoReporte.RESOLVED);

        var resueltos = datos.Count(
            reporte => reporte.Estado == EstadoReporte.RESOLVED);

        var horasResolucion =
            await CalcularHorasResolucionAsync(datos, ct);

        var promedioPorTipo =
            await CalcularPromedioPorTipoCorteAsync(datos, ct);
        ...
        var panel = new EstadisticasPanelDto(
            ...
            resueltos,
            datos.Count(
                reporte => reporte.Estado == EstadoReporte.NEW),
            promedioHoras,
            porEstado,
            porZona,
            promedioPorTipo);
        ...
        CancellationToken ct)
    {
        ...
        var resultado = datos
            .GroupBy(reporte => reporte.ZonaId)
            .Select(grupo =>
                CrearEstadisticaZona(grupo, nombres))
            .ToArray();

        return Respuesta<IReadOnlyCollection<EstadisticasZonaDto>>
            .Correcta(resultado);
    }
        ...
        CancellationToken ct)
    {
        ...
        var tecnicosEncontrados =
            await tecnicos.ListadoAsync(ct);

        var resultado =
            new List<EstadisticasTecnicoDto>();

        foreach (var tecnico in tecnicosEncontrados)
        {
            var usuario =
                await usuarios.PorIdAsync(
                    tecnico.UsuarioId,
                    ct);

            resultado.Add(
                CrearEstadisticaTecnico(
                    tecnico,
                    usuario?.Nombre ?? tecnico.Id,
                    datos));
        }

        return Respuesta<IReadOnlyCollection<EstadisticasTecnicoDto>>
            .Correcta(resultado);
    }
        ...
        CancellationToken ct)
    {
        ...
        if (!agrupacion.Equals(
            ...
            !agrupacion.Equals(
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            return Respuesta<EstadisticasTendenciaDto>
                .Fallida(
                    "La agrupacion debe ser semana o mes",
                    400);
        }

        var reportesPorPeriodo =
            AgruparPorPeriodo(datos, agrupacion)
                .ToDictionary(
                    grupo => grupo.Key,
                    grupo => grupo.Count());

        var resolucionesPorPeriodo =
            new Dictionary<string, int>();

        foreach (var reporte in datos.Where(
                     reporte =>
                         reporte.Estado ==
                         EstadoReporte.RESOLVED))
        {
            var resolucion =
                await resoluciones.PorReporteAsync(
                    reporte.Id,
                    ct);
            ...
            var periodo = ClavePeriodo(
                resolucion.FechaRestablecimiento,
                agrupacion);

            resolucionesPorPeriodo[periodo] =
                resolucionesPorPeriodo
                    .GetValueOrDefault(periodo) + 1;
        }
        ...
        var tendencia =
            new EstadisticasTendenciaDto(
                etiquetas,
                etiquetas
                    .Select(etiqueta =>
                        reportesPorPeriodo
                            .GetValueOrDefault(etiqueta))
                    .ToArray(),
                etiquetas
                    .Select(etiqueta =>
                        resolucionesPorPeriodo
    ...
    private async Task<IReadOnlyCollection<ReporteCorteEnergia>> ObtenerDatosAsync(
        ...
        CancellationToken ct)
    {
        if (filtro.FechaInicial is not null &&
            filtro.FechaFinal is not null &&
    ...
    private async Task<IReadOnlyCollection<PuntoGraficoDto>>
        CalcularPromedioPorTipoCorteAsync(
            IReadOnlyCollection<ReporteCorteEnergia> datos,
            CancellationToken ct)
    {
        var tiempos =
            new List<(string Tipo, double Horas)>();
        ...
        IReadOnlyDictionary<string, string> nombres)
    {
        ...
        var porcentaje = total == 0
            ? 0
            : Math.Round(
                100m * resueltos / total,
                2);

        return new EstadisticasZonaDto(
            grupo.Key,
            nombres.GetValueOrDefault(
                grupo.Key,
                grupo.Key),
            total,
            resueltos,
    ...
    private static IEnumerable<IGrouping<string, ReporteCorteEnergia>>
        AgruparPorPeriodo(
            IReadOnlyCollection<ReporteCorteEnergia> datos,
            string agrupacion)
    {
    ...
    private static string ClavePeriodo(
        DateTime fecha,
        string agrupacion) =>
        agrupacion.Equals(
            "mes",
